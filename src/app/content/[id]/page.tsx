...
export default async function ContentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const accent = accentClasses[character?.accentToken ?? "coral"];
    ...
    <>
      ...
      <section className={`relative overflow-hidden ${accent.bgSoft}`}>
        ...
        <div className="relative mx-auto flex max-w-3xl flex-col items-center gap-5 px-5 py-14 text-center sm:py-20">
          ...
          <h1 className="font-display text-3xl font-semibold text-ink sm:text-4xl">{t(item.title, locale)}</h1>
          <p className="max-w-xl text-lg leading-relaxed text-ink-soft">{t(item.description, locale)}</p>
